...
namespace fast_task::net {
    ...
    void internal_makeIP(universal_address& addr_storage, const char* ip, uint16_t port) {
        init_networking();
        const char* actual_ip = ip;
        char localhost_buf[] = "127.0.0.1";
        if (strcmp(ip, "localhost") == 0)
            actual_ip = localhost_buf;
        sockaddr_in6 addr6;
        memset(&addr6, 0, sizeof(addr6));
        addr6.sin6_family = AF_INET6;
        addr6.sin6_port = htons(port);
        addr6.sin6_addr.s6_addr[10] = 0xFF;
        addr6.sin6_addr.s6_addr[11] = 0xFF;
        if (inet_pton(AF_INET, actual_ip, &addr6.sin6_addr.s6_addr[12]) == 1) {
            memset(&addr_storage, 0, sizeof(addr_storage));
            memcpy(&addr_storage, &addr6, sizeof(addr6));
            return;
        }
        memset(&addr6.sin6_addr, 0, sizeof(addr6.sin6_addr));
        if (inet_pton(AF_INET6, actual_ip, &addr6.sin6_addr) == 1) {
            memset(&addr_storage, 0, sizeof(addr_storage));
            memcpy(&addr_storage, &addr6, sizeof(addr6));
            return;
        }
        throw std::invalid_argument("Invalid ip address (only IP literals and localhost accepted; use address::resolve() for hostnames)");
    }
    ...
    void internal_makeIP4_port(universal_address& addr_storage, const char* ip_port) {
        ...
        if (ip == "localhost") {
            internal_makeIP4(addr_storage, "127.0.0.1", port_num);
            return;
        }
        char first_ch = ip.empty() ? '\0' : ip[0];
        if (!std::isdigit((unsigned char)first_ch))
            throw std::invalid_argument("Invalid ip4 address (only IP literals and localhost accepted; use address::resolve() for hostnames)");
