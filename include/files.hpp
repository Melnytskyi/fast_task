...
namespace fast_task::files {
    ...
    struct FT_API file_flags {
        ...
        bool no_buffering : 1;    //hint to cache manager, ignored with use_lock set to false, affect seek and read write operations, like disc page size aligned operations
        bool write_through : 1;   //hint to cache manager, ignored with use_lock set to false
        bool at_end : 1;
        bool use_lock : 1;
    };
    ...
    public:
        static file_handle open(const std::filesystem::path& path, open_mode open, on_open_action action, file_flags flags = {}, share_mode share = {}, pointer_mode pointer_mode = pointer_mode::combined);
        static file_handle open_throws(const std::filesystem::path& path, open_mode open, on_open_action action, file_flags flags = {}, share_mode share = {}, pointer_mode pointer_mode = pointer_mode::combined);
        ...
        void write_at(uint64_t offset, const uint8_t* data, uint32_t size);
        ...
        future_ptr<std::vector<uint8_t>> fut_read_fixed(uint32_t size);
        future_ptr<std::vector<uint8_t>> fut_read_fixed_at(uint64_t offset, uint32_t size);
        future_ptr<void> fut_write(const uint8_t* data, uint32_t size);
        future_ptr<void> fut_write_at(uint64_t offset, const uint8_t* data, uint32_t size);
        future_ptr<void> fut_append(const uint8_t* data, uint32_t size);


        io_operation<std::vector<uint8_t>> make_read(uint32_t size);
        io_operation<std::vector<uint8_t>> make_read_at(uint64_t offset, uint32_t size);
        io_operation<std::vector<uint8_t>> make_read_fixed(uint32_t size);
        io_operation<std::vector<uint8_t>> make_read_fixed_at(uint64_t offset, uint32_t size);

        io_operation<void> make_write(const uint8_t* data, uint32_t size);
        io_operation<void> make_write_at(uint64_t offset, const uint8_t* data, uint32_t size);
        io_operation<void> make_append(const uint8_t* data, uint32_t size);
        ...
        struct force_static_iofstream {
...
#endif
            inline static file_flags to_flags(std::ios_base::openmode op_mod) {
                file_flags flags{};
            ...
            protected:
                ...
                int flush_buffer() {
                    ...
                    if (size > 0) {
                        _handle.write(reinterpret_cast<const uint8_t*>(pbase()), static_cast<uint32_t>(size));
    ...
    using async_filebuf = detail::force_static_iofstream<true>::async_filebuf;
    using async_iofstream = detail::force_static_iofstream<true>::async_iofstream;
    using atomic_async_ofstream = detail::force_static_iofstream<true>::atomic_async_ofstream; //the writes done to temporary file and replaces the temporary with real one atomically without corrupting the data
}
