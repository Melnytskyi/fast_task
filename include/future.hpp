...
namespace fast_task {
    ...
    extern template struct FT_API future<void>;
    ...
    struct cancelable_future : public future<T> {
        ...
        template <class... Args>
        static std::shared_ptr<cancelable_future> make_ready(Args&&... args)
            requires(!std::is_same_v<T, void>)
        {
            std::shared_ptr<cancelable_future> future_ = std::make_shared<cancelable_future>();
            future_->result = std::make_optional<T>(std::forward<Args>(args)...);
            future_->_is_ready = true;
            return future_;
        }
